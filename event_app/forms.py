...
class ProfileForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'birth_year' : forms.Select(choices=[('', 'Year')] + [(y, y) for y in range(1900, datetime.now().year + 1)], attrs={'class':'form-control fs-4','id':'byear'}),
            'birth_month' : forms.Select(choices=[('', 'Month')] + [(m, month_name[m]) for m in range(1, 13)], attrs={'class':'form-control fs-4','id':'bmonth'}),
            'birth_day' : forms.Select(choices=[('', 'Day')] + [(d, d) for d in range(1, 32)], attrs={'class':'form-control fs-4','id':'bday'}),
            'is_venue_owner' : forms.CheckboxInput(attrs={'class':'form-control fs-4','id':'owner'}),
        }
...
class EventForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'event_year' : forms.Select(choices=[('', 'Year')] + [(y, y) for y in range(1900, datetime.now().year + 1)], attrs={'class':'form-control fs-4','id':'eyear'}),
            'event_month' : forms.Select(choices=[('', 'Month')] + [(m, month_name[m]) for m in range(1, 13)], attrs={'class':'form-control fs-4','id':'emonth'}),
            'event_day' : forms.Select(choices=[('', 'Day')] + [(d, d) for d in range(1, 32)], attrs={'class':'form-control fs-4','id':'eday'}), 
